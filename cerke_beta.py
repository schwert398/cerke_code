...
def main():
    ...
    piece_space = pygame.Surface(Cst.space_size)
    piece_space.fill(Cst.space_color)
    ...
    while True:
        ...
        parameter_list = [Cst.turn[turn], str(score), str(40-score)]    # 常時表示するパラメータ(全てstrオブジェクト)
        ...
        if selector:
            screen.blit(piece_img[choosing[0]][0], (949, 37))
            cur_piece_rect = choosing[1].center
            if cur_piece_rect[0] not in Cst.ntoc_vert or cur_piece_rect[1] not in Cst.ntoc_horz:
                screen.blit(msg_font.render("piece space", True, (0, 0, 0)), (914, 107))
            else:
                tmp = Cst.ntoc_vert[cur_piece_rect[0]] + Cst.ntoc_horz[cur_piece_rect[1]]
                screen.blit(msg_font.render(tmp, True, (0, 0, 0)), (914, 107))
        else:
            screen.blit(msg_font.render("not selecting", True, (0, 0, 0)), (914, 107))
        ...
        if tmp is None:
            ...
            ret = Process.process_str(tmp, board_info, piece_img, undo_repository, msg_queue, turn, selector, choosing, score)
            board_info, undo_repository, msg_queue, turn, selector, choosing, score = ret

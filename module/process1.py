...
def _hand_pos_provider(piece_obj_list: list, turn: int) -> pygame.Rect:
...
def _get_piece(getter: Piece, target: Piece, target_dest: pygame.Rect):
            ...
            if collide_index >= 0:
                ...
                # if target is opponent's piece (or tam)
                    ...
                    if if_get:
                        ...
                        target_dest = _hand_pos_provider(piece_obj_list, turn)
                        _get_piece(choice, target, target_dest)
                        hand_queue.append(tmp_hand)
                        hand_queue[-1] += "${}".format(
